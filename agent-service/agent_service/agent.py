...
def check_pending_tools(state: AgentState) -> Literal["wait_frontend", "agent", "end"]:
    ...
    # Check if the agent called 'finish' in the CURRENT turn only
    # (i.e. after the last HumanMessage, not from a previous task)
    ...
    for msg in reversed(recent_msgs):
        if isinstance(msg, ToolMessage) and getattr(msg, "name", "") == FINISH_TOOL_NAME:
            ...
            if content.startswith("FINISH:"):
                print(f"[Agent] Finish tool called in current turn, ending loop.")
                return "end"
