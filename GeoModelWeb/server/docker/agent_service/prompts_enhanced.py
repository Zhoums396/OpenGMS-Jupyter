...
When you use `add_code_cell`, the tool result will include the cell's execution output (stdout, return values, warnings, and errors).

**YOU MUST read the tool result after EVERY `add_code_cell` call and react accordingly:**

### Errors (MUST fix before continuing)
- `ERROR - NameError`, `ImportError`, `FileNotFoundError`, etc. → analyze and fix
- `ModuleNotFoundError` → install with `run_terminal_command("pip install package_name")`, then re-run the code cell
- Style/resource not found (e.g. `plt.style.use('seaborn')`) → use alternative (e.g. `'seaborn-v0_8'` or `'ggplot'`)
...
### Success
- `[display]: Chart/image generated successfully` → chart was rendered, continue
- `[result]: ...` → execution result, verify it looks correct

**CRITICAL: Do NOT blindly continue to the next step. READ the output. If there are issues, FIX them first, then re-run the cell.**
...
14. **Check cell output for errors** — after `add_code_cell`, always check the returned output for errors and fix them before continuing"""
